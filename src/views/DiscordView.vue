...
onMounted(() => {
  ...
  if (id) {
    if (store.state.userInput.discordAccounts[id]) {
      state.obj = { ...store.state.userInput.discordAccounts[id] }
    }
  }
})
...
const resolver = () => {
  ...
  if (!state.obj.discordName) {
    errors.discordName = [{ message: 'Discord id is required.' }]
  }
...
const onFormSubmit = ({ valid }) => {
  ...
  if (valid) {
    ...
    if (!id) id = uuidv7()
    store.state.userInput.discordAccounts[id] = state.obj
      ...
      >
        ...
        >
          <InputTextField
            ...
            v-model="state.obj.discordName"
            helpText="Discord user name. For example BillGates#1234"
            labelText="Discord user name"
          />
        </Fieldset>

        <Button type="submit" severity="primary" label="Save the discord" />
