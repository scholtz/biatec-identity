...
import type { ICompanyAddress } from '@/interface/ICompanyAddress'
...
const onFormSubmit = ({ valid }) => {
  ...
  if (valid) {
    const id = uuidv7()
    if (!store.state.userInput.companyAddresses) store.state.userInput.companyAddresses = {}
    store.state.userInput.companyAddresses[id] = state.obj
...
const defaultObj: ICompanyAddress = {
  companyId: '',
  addressId: '',
  type: undefined,
  note: ''
...
<template>
  <Layout>
    <Form v-slot="$form" :resolver @submit="onFormSubmit" class="flex flex-col gap-4 w-full">
      <Fieldset
        legend="Assign address to the company"
      ...
      >
        ...
        >
          <Fieldset
            ...
            pt:content:class="flex gap-2 flex flex-col"
          >
            <div v-for="(companyAddress, id) in store.state.userInput.companyAddresses">
              <div v-if="companyAddress.addressId == state.obj.addressId">
                <span>
                  {{ companyFormatter(store.state.userInput.companies[companyAddress.companyId]) }}
                  - {{ enumFormatter('COMPANY_ADDRESS_TYPE', companyAddress.type) }} -
                  {{ companyAddress.note ? companyAddress.note : 'No comment' }}
                </span>
                <Button
                  severity="secondary"
                  @click="delete store.state.userInput.companyAddresses[id]"
                  class="m-2"
                >
                  X
                </Button>
          ...
          <EnumSelectField
            ...
            labelText="Company to address relation"
            enum="COMPANY_ADDRESS_TYPE"
          />

          <EnumSelectField
            fieldName="companyId"
            v-model="state.obj.companyId"
            helpText="Select company who is related to the address."
            labelText="Company"
            enum="COMPANY"
          />
