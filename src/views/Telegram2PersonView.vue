...
import type { IPersonTelegram } from '@/interface/IPersonTelegram'
...
const onFormSubmit = ({ valid }) => {
  ...
  if (valid) {
    const id = uuidv7()
    if (!store.state.userInput.personTelegramAccounts)
      store.state.userInput.personTelegramAccounts = {}
    store.state.userInput.personTelegramAccounts[id] = state.obj
...
const defaultObj: IPersonTelegram = {
  personId: '',
  telegramId: '',
  note: ''
}
...
<template>
  <Layout>
    <Form v-slot="$form" :resolver @submit="onFormSubmit" class="flex flex-col gap-4 w-full">
      <Fieldset
        legend="Assign telegram to the person"
      ...
      >
        ...
        >
          <Fieldset
            ...
            pt:content:class="flex gap-2 flex flex-col"
          >
            <div v-for="(personTelegram, id) in store.state.userInput.personTelegramAccounts">
              <div v-if="personTelegram.telegramId == state.obj.telegramId">
                <span>
                  {{ personFormatter(store.state.userInput.people[personTelegram.personId]) }} -
                  {{ personTelegram.note ? personTelegram.note : 'No comment' }}
                </span>
                <Button
                  severity="secondary"
                  @click="delete store.state.userInput.personTelegramAccounts[id]"
                  class="m-2"
                >
                  X
                </Button>
          ...
          <EnumSelectField
            ...
            helpText="Select person who is related to the telegram."
            labelText="Person"
            enum="PERSON"
          />
