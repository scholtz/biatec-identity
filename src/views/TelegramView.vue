...
onMounted(() => {
  ...
  if (id) {
    if (store.state.userInput.telegramAccounts[id]) {
      state.obj = { ...store.state.userInput.telegramAccounts[id] }
    }
  }
})
...
const resolver = () => {
  ...
  if (!state.obj.telegramAccount) {
    errors.telegramAccount = [{ message: 'Telegram account is required.' }]
  }
...
const onFormSubmit = ({ valid }) => {
  ...
  if (valid) {
    ...
    if (!id) id = uuidv7()
    store.state.userInput.telegramAccounts[id] = state.obj
      ...
      >
        ...
        >
          <InputTextField
            ...
            v-model="state.obj.telegramAccount"
            helpText="Telegram account name."
            labelText="Telegram account"
          />
        </Fieldset>

        <Button type="submit" severity="primary" label="Save the telegram" />
