...
import type { IPersonDiscord } from '@/interface/IPersonDiscord'
...
const onFormSubmit = ({ valid }) => {
  ...
  if (valid) {
    const id = uuidv7()
    if (!store.state.userInput.personDiscordAccounts)
      store.state.userInput.personDiscordAccounts = {}
    store.state.userInput.personDiscordAccounts[id] = state.obj
...
const defaultObj: IPersonDiscord = {
  personId: '',
  discordId: '',
  note: ''
}
...
<template>
  <Layout>
    <Form v-slot="$form" :resolver @submit="onFormSubmit" class="flex flex-col gap-4 w-full">
      <Fieldset
        legend="Assign discord to the person"
      ...
      >
        ...
        >
          <Fieldset
            ...
            pt:content:class="flex gap-2 flex flex-col"
          >
            <div v-for="(personDiscord, id) in store.state.userInput.personDiscordAccounts">
              <div v-if="personDiscord.discordId == state.obj.discordId">
                <span>
                  {{ personFormatter(store.state.userInput.people[personDiscord.personId]) }} -
                  {{ personDiscord.note ? personDiscord.note : 'No comment' }}
                </span>
                <Button
                  severity="secondary"
                  @click="delete store.state.userInput.personDiscordAccounts[id]"
                  class="m-2"
                >
                  X
                </Button>
          ...
          <EnumSelectField
            ...
            helpText="Select person who is related to the discord."
            labelText="Person"
            enum="PERSON"
          />
