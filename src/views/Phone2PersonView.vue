...
import type { IPersonPhone } from '@/interface/IPersonPhone'
...
const onFormSubmit = ({ valid }) => {
  ...
  if (valid) {
    const id = uuidv7()
    if (!store.state.userInput.personPhones) store.state.userInput.personPhones = {}
    store.state.userInput.personPhones[id] = state.obj
...
<template>
  <Layout>
    <Form v-slot="$form" :resolver @submit="onFormSubmit" class="flex flex-col gap-4 w-full">
      <Fieldset
        legend="Assign phone to the person"
      ...
      >
        ...
        >
          <Fieldset
            ...
            pt:content:class="flex gap-2 flex flex-col"
          >
            <div v-for="(personPhone, id) in store.state.userInput.personPhones">
              <div v-if="personPhone.phoneId == state.obj.phoneId">
                <span>
                  {{ personFormatter(store.state.userInput.people[personPhone.personId]) }} -
                  {{ personPhone.note ? personPhone.note : 'No comment' }}
                </span>
                <Button
                  severity="secondary"
                  @click="delete store.state.userInput.personPhones[id]"
                  class="m-2"
                >
                  X
                </Button>
          ...
          <EnumSelectField
            ...
            helpText="Select person who is related to the phone."
            labelText="Person"
            enum="PERSON"
          />
